    ...
    <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200/50 sticky top-0 z-50 transition-all shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          {/* Desktop Nav - Cambiado a 'sm:flex' para asegurar fila horizontal estable en pantallas normales y laptops */}
          <div className="hidden sm:flex items-center gap-4 md:gap-6">
            {!user && (
              <>
                <Link to="/" className="text-slate-600 hover:text-secondary-500 font-medium transition-colors">Inicio</Link>
                <Link to="/login" className="bg-secondary-500 text-white px-4 py-2 md:px-5 md:py-2.5 rounded-full font-medium hover:bg-secondary-600 hover:shadow-lg hover:-translate-y-0.5 transition-all text-sm md:text-base">
            ...
            {user && (
              <div className="flex items-center gap-3 md:gap-4 ml-2 md:ml-4 border-l pl-4 md:pl-6 border-slate-200">
                <div className="flex items-center gap-2">
                  <div className="bg-slate-100 p-1.5 rounded-full text-slate-500">
                    <UserIcon size={18} />
                  </div>
                  <span className="text-xs md:text-sm text-slate-600 font-medium max-w-[100px] truncate">{user.nombre}</span>
                </div>
                <button 
                  onClick={handleLogout}
                  className="flex items-center gap-1 text-xs md:text-sm text-slate-500 font-medium hover:text-red-600 transition-colors"
                  title="Cerrar Sesión"
                >
                  <LogOut size={18} />
          ...
          {/* Mobile Menu Toggle - Se oculta a partir de 'sm' para acoplarse a la barra horizontal */}
      ...
      {isMobileMenuOpen && (
        <div className="sm:hidden bg-white border-t border-slate-100 px-4 pt-2 pb-6 space-y-3 shadow-lg absolute w-full left-0">
          {!user ? (
            <div className="flex flex-col gap-4 mt-2">
              <Link onClick={() => setIsMobileMenuOpen(false)} to="/" className="text-slate-600 font-medium block">Inicio</Link>
              <Link onClick={() => setIsMobileMenuOpen(false)} to="/login" className="bg-secondary-500 text-white px-5 py-3 rounded-xl font-medium text-center mt-2">
                Inicia sesión / Regístrate
              </Link>
            </div>
          ) : (
